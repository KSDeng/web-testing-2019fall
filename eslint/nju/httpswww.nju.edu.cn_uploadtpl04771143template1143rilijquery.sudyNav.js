...
//(function($) {
    var initSelected = function(){
		$(".menu-item,.wp_nav").each(function(){
			var arr = $(this).find("a");
			for(let i=0 ;i<arr.length;i++){
				if(arr[i].href===window.location.href){
					$(arr[i].parentNode).addClass("selected").siblings().removeClass("selected");
					$(this).addClass("parent").siblings().removeClass("parent");
					break;
				}
			}
		});
		...
		function() {
			...
			for (let i = 0; i < arr.length; i++) {
				var arra = $(arr[i]).find("a");
				for (let j = 0; j < arra.length; j++) {
					if (arra[j].href === window.location.href) {
						$(arr[i]).addClass("selected").siblings().removeClass("selected");
						break;
					}
				}
